    ...
    // Read back using WAL-like chunked reading pattern
    {
        ...
        while (true) {
            ...
            if (chunk_num == 1) {
                ...
                log.info("Chunk 1 header: checksum=0x{X}, type={}, payload_size={}", .{
                    header_checksum, header_type, header_payload_size
                });
                ...
                if (bytes_read > 110) {
                    ...
                    if (read_buffer[100] != expected_at_100) {
                        log.err("CORRUPTION at offset 100: expected 0x{X}, got 0x{X}", .{
                            expected_at_100, read_buffer[100]
                        });
            ...
            for (read_buffer[0..bytes_read], total_read - bytes_read..) |byte, global_offset| {
                ...
                if (byte != expected_byte) {
                    log.err("CORRUPTION at global offset {}: expected 0x{X}, got 0x{X}", .{
                        global_offset, expected_byte, byte
                    });
                    return error.CorruptionDetected;
                }
            }
