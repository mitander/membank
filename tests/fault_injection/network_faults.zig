...
test "server config validation" {
    ...
    for (config_tests) |test_case| {
        var server = Server.init(allocator, test_case.config, harness.storage_harness.storage_engine, harness.query_engine);
        defer server.deinit();
...
test "concurrent server configuration" {
    ...
    for (configs) |config| {
        var server = Server.init(allocator, config, harness.storage_harness.storage_engine, harness.query_engine);
        defer server.deinit();
