...
test "full data lifecycle with compaction (migrated)" {
    ...
    var harness = try kausaldb.SimulationHarness.init_and_startup(allocator, 0x5EC7E571, // Same deterministic seed for identical behavior
    ...
    for (1..edge_count + 1) |i| {
        if (i + 1 <= num_blocks) {
            ...
            const edge = kausaldb.TestData.create_test_edge(@intCast(i), @intCast(i + 1), .calls);
...
test "lifecycle performance regression detection" {
    ...
    for (0..10) |_| { // 10 measurement samples
        ...
        harness.storage_engine.* = try kausaldb.StorageEngine.init_default(harness.arena.allocator(), harness.node().filesystem_interface(), "perf_test_reset");
        try harness.storage_engine.startup();
    }

    // Assert performance using tier-aware thresholds
    try batch_measurements.assert_statistics("bulk_ingestion_performance", 25_000_000 * bulk_blocks, // 25ms per block baseline * block count
